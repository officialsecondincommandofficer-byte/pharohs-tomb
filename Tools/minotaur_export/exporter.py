...
def teleport_pair_dictionary(pair: TeleportPair) -> str:
    return '{%s}' % ", ".join(
        [
            f'"a": {vector2i(pair.a)}',
            f'"b": {vector2i(pair.b)}',
...
def teleport_pair_array(values: Iterable[TeleportPair]) -> str:
    serialized = ", ".join(teleport_pair_dictionary(value) for value in values)
    return f"Array[Dictionary]([{serialized}])"
...
class GodotMazeExporter:
    ...
    def serialize(
        ...
        lines = [
            ...
            f"player_horizontal_walls = {vector2i_array(partition_walls(record.player_only_walls)[0])}",
            ...
            f"main_exit_cell = {vector2i(record.main_exit_cell)}",
            f"exit_cells = {vector2i_array(record.resolved_goal_cells)}",
            f"main_exit_cells = {vector2i_array(record.main_exit_cells)}",
            f"win_zone_cells = {vector2i_array(record.win_zone_cells)}",
            f"escape_zone_cells = {vector2i_array(record.escape_zone_cells)}",
            f"zone_spawners = {zone_spawner_array(record.zone_spawners)}",
            f"escape_zone_spawners = {zone_spawner_array(record.escape_zone_spawners)}",
